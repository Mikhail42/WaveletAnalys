...
object Operation {
 ...
 /* def toGray(img: BI): BI = {
  ...
  def toColorMInt(mat: M): MInt = map(mat, (x: T) => x.toInt.max(0).min(255))
  ...
  def toBinary(img: BI, white: Int = 160): BI = {
    ...
    val mat = map(getMInt(img), (x: Int) => if (x<white) 0 else 255)
  ...
  def correlation(img1: BI, img2: BI, colorID: Int): T = {
    val mat1 = map(Input.getColorsComponents(img1, colorID), (x: Int) => x.toDouble) 
    val mat2 = map(Input.getColorsComponents(img2, colorID), (x: Int) => x.toDouble)
  ...
  def disp(img: BI, colorID: Int): T = {
    val mat = map(Input.getColorsComponents(img, colorID), (x: Int) => x.toDouble)
