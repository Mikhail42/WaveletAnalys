...
object Transform {
  ...
  def DaubechiesForwardImage(img: BI, order: Int, trans: String): BI = {
    ...
    val mat: M = 
      map(getColorsComponents(img, 2), (x: Int) => x.toDouble)
    val g: MInt = 
      map(transform.DTransform.daubechies(mat, order, trans), (x: T) => x.toInt) 
  ...
  def cwt(wave: wavelets.ICWavelet, a: T, id: Int= 1, img: BI): (BI, BI, BI) = {
    ...
    def locTr2(theta: Int): M = {
      ...
      val matR: M = getM(imgR) 
      val matTrR: MInt = map(trMor(matR), (x: T) => x.toInt)
